...
function blastOrders(view, boss = false) {
  ...
  const waits = Math.min(26, Math.ceil(Math.max(0, view.now.blastReadyInMs - 50) / (1000 / 30)));
...
function selfCheck() {
  ...
  assert.equal(blastOrders({ now: { blastReadyInMs: 900, threats: { alive: 1, edge: 'north' } } }).length, 28);
  assert.deepEqual(blastTarget({ now: { threats: {} } }, true), { x: 0, z: 11 });
