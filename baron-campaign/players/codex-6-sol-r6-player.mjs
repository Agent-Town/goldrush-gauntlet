...
function palisadeGoal(view) {
  if (view.now.wave < 19) return null;
  const built = view.now.works.byKind.palisade ?? 0;
  return built < PALISADES.length
    ? { cost: COSTS.palisade, pos: PALISADES[built], order: { verb: 'BUILD', what: 'palisade', where: PALISADES[built], when: { goldGte: COSTS.palisade } } }
    : null;
}
...
function blastOrders(view, boss = false) {
  ...
  const pos = blastTarget(view, boss);
  const waitTicks = Math.min(26, Math.ceil(view.now.blastReadyInMs / (1000 / 30)));
  const orders = Array.from({ length: waitTicks }, () => ({ verb: 'SET_WEAPON', weapon: 'rig' }));
  orders.push({ verb: 'BLAST_AT', pos });
  if (waitTicks < 26) orders.push({ verb: 'BLAST_AT', pos });
  return orders;
}
...
function ordersFor(view) {
  ...
  if (view.now.wave >= 19) {
    const goal = palisadeGoal(view) ?? (view.now.wave < 20 ? planGoal(view) : null);
    if (goal) {
...
function selfCheck() {
  ...
  assert.equal(blastOrders({ now: { blastReadyInMs: 2500, threats: { alive: 1, edge: 'north' } } }).length, 27);
  assert.deepEqual(palisadeGoal({ now: { wave: 19, works: { byKind: { palisade: 0 } } } }).pos, PALISADES[0]);
