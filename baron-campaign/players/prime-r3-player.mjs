...
const HOLD = { x: 0, z: 12 };
// Blocker-centered charges preserve add control while the Spark Rig stays continuously active.
const BARON_STRIKE = { x: 0, z: 11 };
...
function phaseGoals(view) {
  const desiredPalisades = view.now.wave >= 19 ? 3 : 0;
  if ((view.now.works.byKind.palisade ?? 0) < desiredPalisades) {
    return Array.from({ length: desiredPalisades - (view.now.works.byKind.palisade ?? 0) }, (_, offset) => {
      const built = (view.now.works.byKind.palisade ?? 0) + offset;
      return { cost: COSTS.palisade, pos: PALISADES[built], order: { verb: 'BUILD', what: 'palisade', where: PALISADES[built], when: { goldGte: COSTS.palisade } } };
    });
  }
  if (view.now.wave >= 19) return [];
...
function blastOrders(view, boss = false) {
  ...
  if (boss) {
    // The two shooters are weapon-gated separately. Stay on rig, spend idempotent SETs as a
    // fixed-step cooldown wait, then throw explicitly. The duplicate throw intentionally fails
    // one tick after a success and opens the next deterministic observation.
    const waits = Math.min(26, Math.max(0, Math.ceil(view.now.blastReadyInMs / (1000 / 30)) - 1));
    return [
      ...Array.from({ length: waits }, () => ({ verb: 'SET_WEAPON', weapon: 'rig' })),
      { verb: 'BLAST_AT', pos: BARON_STRIKE }, { verb: 'BLAST_AT', pos: BARON_STRIKE },
    ];
  }
  const edge = view.now.threats.edge;
  const pos = edge === 'north' ? { x: 0, z: 20 }
    : edge === 'south' ? { x: 0, z: 4 }
      : edge === 'east' ? { x: 8, z: 12 }
        : edge === 'west' ? { x: -8, z: 12 }
          : { x: 0, z: 9 };
...
function selfCheck() {
  ...
  assert.equal(blastOrders({ now: { blastReadyInMs: 0, threats: { alive: 1, edge: 'north' } } }).length, 2);
  const bossOrders = blastOrders({ now: { blastReadyInMs: 0, threats: { alive: 1 } } }, true);
  assert.equal(bossOrders[0].verb, 'BLAST_AT');
  assert.deepEqual(bossOrders[0].pos, BARON_STRIKE);
