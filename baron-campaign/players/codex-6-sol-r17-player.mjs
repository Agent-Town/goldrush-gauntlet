...
const PLAN = [
  ...
  { type: 'build', id: 'stockpile', index: 0, pos: STOCKPILE, cost: COSTS.stockpile },
...
function selfCheck() {
  ...
  assert.equal(PLAN.filter(({ id }) => id === 'sentry_beacon').length, 5);
  assert.equal(PLAN.filter(({ id }) => id === 'stockpile').length, 1);
