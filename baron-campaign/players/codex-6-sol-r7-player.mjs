...
function phaseGoals(view) {
  ...
  if (builtPalisades < desiredPalisades) {
    return Array.from({ length: desiredPalisades - builtPalisades }, (_, offset) => {
      const built = builtPalisades + offset;
    ...
    : edge === 'north' ? { x: 0, z: 20 }
      : edge === 'south' ? { x: 0, z: 4 }
        : edge === 'east' ? { x: 8, z: 12 }
          : edge === 'west' ? { x: -8, z: 12 }
            : { x: 0, z: 9 };
  const waits = Math.min(26, Math.max(0, Math.ceil((view.now.blastReadyInMs ?? 0) / (1000 / 30))));
...
function selfCheck() {
  ...
  assert.equal(blastOrders({ now: { blastReadyInMs: 1000, threats: { alive: 1, edge: 'north' } } }).length, 28);
  assert.deepEqual(blastOrders({ now: { blastReadyInMs: 0, threats: { alive: 1 } } }, true)[0].pos, { x: 0, z: 11 });
  assert.ok(BEACONS.every(({ z }) => z >= 14));
