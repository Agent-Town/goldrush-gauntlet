...
const upgrade = (id, index, fromTier) => ({ type: 'upgrade', id, index, fromTier, cost: TIER_COSTS[id][fromTier] });
const PLAN = [
  ...
  ...SLUICES.flatMap((_, index) => [upgrade('sluice', index, 1), upgrade('sluice', index, 2)]),
  build('sentry_beacon', 1, BEACONS), build('sentry_beacon', 2, BEACONS),
  ...TURRETS.map((_, index) => upgrade('turret', index, 1)),
  ...TURRETS.map((_, index) => upgrade('turret', index, 2)),
  ...BEACONS.slice(3).map((_, offset) => build('sentry_beacon', offset + 3, BEACONS)),
  build('stockpile', 1, STOCKPILES),
];

function pickUpgrade(view) {
  const hurt = view.now.hero.hp / Math.max(1, view.now.hero.maxHp) < 0.72;
  const scores = {
    ...
    beacon_handoff: 104, tinkers_plating: view.now.wave >= 16 || hurt ? 145 : 101,
    sharpen: 100, wide_ring: 94, spark_pressure_ring: 92,
    field_dressing: view.now.wave >= 18 || hurt ? 142 : 60,
...
function moneyOrders(view) {
  ...
  const locations = new Map(view.stablePrefix.map.seams.map((seam) => [seam.id, seam]));
  const prospector = view.now.prospector ?? HOLD;
  const seam = view.now.seams.filter(({ active, remaining }) => active && remaining > 0).sort((a, b) => {
    const left = locations.get(a.id);
    const right = locations.get(b.id);
...
function selfCheck() {
  const choice = pickUpgrade({ now: { wave: 10, hero: { hp: 100, maxHp: 100 }, pendingOffer: [{ id: 'spring_heels' }, { id: 'heavy_spark' }] } });
  assert.equal(choice.id, 'heavy_spark');
  const orders = blastOrders({ now: { blastReadyInMs: 500, threats: { alive: 1, edge: 'north' } } });
  assert.equal(orders.at(-1).verb, 'BLAST_AT');
  assert(orders.slice(0, -2).every((order) => order.verb === 'SET_WEAPON' && order.weapon === 'rig'));
  assert(orders.length <= 28);
...
child.stdout.on('data', (chunk) => {
  ...
  while ((newline = buffer.indexOf('\n')) >= 0) {
    ...
    if (message.schema === 'goldrush.view.v1') {
      ...
      transcript.write(`${JSON.stringify({ type: 'turn', wave: message.now.wave, gold: message.now.gold, hero: message.now.hero, threats: message.now.threats, works: message.now.works, orders })}\n`);
