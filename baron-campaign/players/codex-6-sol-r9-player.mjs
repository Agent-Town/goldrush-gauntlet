...
function ordersFor(view) {
  ...
  if (wrecked) {
    const harvest = moneyOrders(view);
    if (view.now.gold >= wrecked.repairCost) return [...prefix, { verb: 'REPAIR_UNDER', pct: 1 }, ...blastOrders(view)].slice(0, 32);
    if (harvest.length) return [...prefix, ...harvest].slice(0, 32);
  }
...
function selfCheck() {
  ...
  assert.equal(blastOrders({ now: { blastReadyInMs: 500, threats: { alive: 1 } } }, true)[0].weapon, 'rig');
