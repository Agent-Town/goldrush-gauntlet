...
function pickUpgrade(view) {
  ...
  const scores = {
    split_spark: 130, heavy_spark: 126, double_tap_coil: 123, long_resonator: 119,
    powder_charge: 117, quick_fuse: 115, beacon_dynamo: 113, chain_spark_arc: 111,
    beacon_handoff: 109, sharpen: 107, wide_ring: 105, spark_pressure_ring: 103,
    tinkers_plating: hpRatio < 0.55 ? 145 : 82,
    field_dressing: hpRatio < 0.45 ? 142 : 60,
...
function phaseGoal(view) {
  ...
  if (view.now.wave >= 20) return null;
...
function blastOrders(view, boss = false) {
  ...
  const waitTicks = Math.min(26, Math.max(0, Math.ceil((view.now.blastReadyInMs ?? 0) / (1000 / 30))));
  return [...Array.from({ length: waitTicks }, () => ({ verb: 'SET_WEAPON', weapon: 'rig' })), blast, blast];
}
...
function ordersFor(view) {
  ...
  const picked = view.now.pendingOffer?.length ? pickUpgrade(view) : null;
  const prefix = picked ? [{ verb: 'PICK_UPGRADE', id: picked.id }] : [];
  const entries = view.now.works.entries;
  ...
  if (wrecked) {
    const harvest = moneyOrders(view);
    if (view.now.gold >= repairCostEstimate(wrecked)) return [...prefix, { verb: 'REPAIR_UNDER', pct: 1 }, ...blastOrders(view)].slice(0, 32);
    if (harvest.length) return [...prefix, ...harvest].slice(0, 32);
  }
  const goal = phaseGoal(view);
...
function selfCheck() {
  const view = { now: { wave: 18, hero: { hp: 90, maxHp: 100 }, pendingOffer: [{ id: 'tinkers_plating' }, { id: 'chain_spark_arc' }] } };
  assert.equal(pickUpgrade(view).id, 'chain_spark_arc');
  view.now.hero.hp = 50;
  assert.equal(pickUpgrade(view).id, 'tinkers_plating');
  assert.equal(blastOrders({ now: { blastReadyInMs: 2500, threats: { alive: 1 } } }, true).length, 28);
  assert.equal(BEACONS.length, 6);
...
child.stdout.on('data', (chunk) => {
  ...
  while ((newline = buffer.indexOf('\n')) >= 0) {
    ...
    if (message.schema === 'goldrush.view.v1') {
      ...
      transcript.write(`${JSON.stringify({ type: 'turn', wave: message.now.wave, timeMs: message.now.timeMs, gold: message.now.gold, hero: message.now.hero, threats: message.now.threats, offer: message.now.pendingOffer, stacks: message.now.upgrades, works: message.now.works, orders })}\n`);
