...
const BEACONS = [{ x: 0, z: 15 }, { x: -6, z: 14 }, { x: -4, z: 15 }, { x: 2, z: 15 }, { x: 3, z: 14 }, { x: -2, z: 15 }];
...
function phaseGoal(view) {
  ...
  if (view.now.wave >= 19) {
    const pos = PALISADES.find((candidate) => !entries.some((entry) => entry.id === 'palisade' && Math.abs(entry.position.x - candidate.x) < 0.2 && Math.abs(entry.position.z - candidate.z) < 0.2));
    return pos ? { cost: 10, pos, order: { verb: 'BUILD', what: 'palisade', where: pos, when: { goldGte: 10 } } } : null;
  }
...
function combatOrders(view, boss = false) {
  if (view.now.threats.alive === 0) return [{ verb: 'SET_WEAPON', weapon: 'rig' }, { verb: 'HOLD', pos: HOLD }];
  const edge = view.now.threats.edge;
  const pos = boss ? PALISADES[0]
    : edge === 'north' ? { x: 0, z: 20 }
      : edge === 'south' ? { x: 0, z: 4 }
        : edge === 'east' ? { x: 8, z: 12 }
          : edge === 'west' ? { x: -8, z: 12 }
            : { x: 0, z: 9 };
  const waitTicks = Math.min(boss ? 10 : 26, Math.max(0, Math.ceil(view.now.blastReadyInMs / (1000 / 30))));
  const wait = Array.from({ length: waitTicks }, () => ({ verb: 'SET_WEAPON', weapon: 'rig' }));
  const blast = { verb: 'BLAST_AT', pos };
  return [...wait, blast, blast, { verb: 'HOLD', pos: HOLD }];
}
...
function ordersFor(view) {
  ...
  if (view.now.wave >= 19) {
    ...
    if (goal) {
      if (view.now.gold >= goal.cost) return [...prefix, { verb: 'MOVE_TO', pos: goal.pos }, goal.order, ...combatOrders(view, true)].slice(0, 32);
      const harvest = moneyOrders(view);
      if (harvest.length) return [...prefix, ...harvest].slice(0, 32);
    }
    const center = entries.find(({ id, index }) => id === 'palisade' && index === 0);
    if (center && (center.wrecked || center.hp / center.maxHp < 0.31)) {
      return [...prefix, { verb: 'REPAIR_UNDER', pct: 31 }, ...combatOrders(view, true)].slice(0, 32);
    }
    return [...prefix, ...combatOrders(view, true)].slice(0, 32);
  }

  const wrecked = entries.find(({ id, wrecked: isWrecked }) => isWrecked && id !== 'palisade');
  if (wrecked) {
    const harvest = moneyOrders(view);
    if (view.now.gold >= repairCostEstimate(wrecked)) return [...prefix, { verb: 'REPAIR_UNDER', pct: 1 }, ...combatOrders(view)].slice(0, 32);
    if (harvest.length) return [...prefix, ...harvest].slice(0, 32);
  }
  const goal = phaseGoal(view);
  if (goal && view.now.gold >= goal.cost) return [...prefix, { verb: 'MOVE_TO', pos: goal.pos }, goal.order, ...combatOrders(view)].slice(0, 32);
  const harvest = moneyOrders(view);
  if (harvest.length) return [...prefix, ...harvest].slice(0, 32);
  return [...prefix, ...combatOrders(view)].slice(0, 32);
}

function selfCheck() {
  const view = { now: { wave: 20, blastReadyInMs: 500, hero: { hp: 100, maxHp: 100 }, threats: { alive: 1, edge: 'north' }, pendingOffer: [{ id: 'spring_heels' }, { id: 'heavy_spark' }] } };
  assert.equal(pickUpgrade(view).id, 'heavy_spark');
  assert.equal(combatOrders(view, true).filter(({ verb }) => verb === 'SET_WEAPON').length, 10);
  assert.deepEqual(combatOrders(view, true).find(({ verb }) => verb === 'BLAST_AT').pos, PALISADES[0]);
...
const child = spawn(process.execPath, ['scripts/gr-sim.mjs', '--contract', 'e1-baron', '--seed', 'e1-baron-01', '--tape', tapePath], { cwd: ROOT, stdio: ['pipe', 'pipe', 'pipe'] });
let buffer = '';
let outcome;
...
child.stdout.on('data', (chunk) => {
  ...
  while ((newline = buffer.indexOf('\n')) >= 0) {
    ...
    if (message.schema === 'goldrush.view.v1') {
      ...
      transcript.write(`${JSON.stringify({ type: 'turn', wave: message.now.wave, timeMs: Math.round(message.now.timers.runSeconds * 1000), gold: message.now.gold, hero: message.now.hero, threats: message.now.threats, works: message.now.works, orders })}\n`);
