...
function phaseGoal(view) {
  ...
  if (view.now.wave >= 19 && palisades < PALISADES.length) {
    return { cost: COSTS.palisade, pos: PALISADES[palisades], order: { verb: 'BUILD', what: 'palisade', where: PALISADES[palisades], when: { goldGte: COSTS.palisade } } };
  }
...
function blastTarget(view, boss) {
  ...
  return view.now.threats.edge === 'north' ? { x: 0, z: 20 }
    : view.now.threats.edge === 'south' ? { x: 0, z: 4 }
      : view.now.threats.edge === 'east' ? { x: 8, z: 12 }
        : view.now.threats.edge === 'west' ? { x: -8, z: 12 }
          : { x: 0, z: 9 };
}
...
function blastOrders(view, boss = false) {
  ...
  return [...Array.from({ length: waits }, () => ({ verb: 'SET_WEAPON', weapon: 'rig' })), blast, blast];
}
...
function ordersFor(view) {
  ...
  const harvest = moneyOrders(view);
  return harvest.length ? [...prefix, ...harvest].slice(0, 32) : [...prefix, ...blastOrders(view)].slice(0, 32);
}
...
function selfCheck() {
  ...
  const orders = blastOrders({ now: { blastReadyInMs: 2500, threats: { alive: 1, edge: 'north' } } });
  assert.equal(orders.length, 28);
  assert.equal(orders[0].weapon, 'rig');
