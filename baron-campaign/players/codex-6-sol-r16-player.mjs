...
function phaseGoals(view) {
  ...
  if (view.now.wave >= 19 && palisadeCount < PALISADES.length) {
    return PALISADES.slice(palisadeCount).map((pos) => ({
      cost: COSTS.palisade,
      pos,
      order: { verb: 'BUILD', what: 'palisade', where: pos, when: { goldGte: COSTS.palisade } },
    }));
  }
...
function blastOrders(view, boss = false) {
  ...
  if (boss) {
    const wait = Math.min(26, Math.max(1, Math.ceil(view.now.blastReadyInMs / (1000 / 30))));
    return [...Array.from({ length: wait }, () => ({ verb: 'SET_WEAPON', weapon: 'rig' })),
      { verb: 'BLAST_AT', pos: { x: 0, z: 11 } }, { verb: 'BLAST_AT', pos: { x: 0, z: 11 } }];
  }
...
function selfCheck() {
  ...
  assert.equal(blastOrders({ now: { blastReadyInMs: 800, threats: { alive: 1 } } }, true).at(-1).verb, 'BLAST_AT');
  assert.equal(PLAN.filter(({ type, id }) => type === 'build' && id === 'sentry_beacon').length, 5);
