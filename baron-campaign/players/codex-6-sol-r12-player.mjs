...
const run = selfCheckMode ? 1 : Number(process.argv[2] ?? 1);
...
const atPosition = (entry, step) => entry.id === step.id
    ...
    : edge === 'north' ? { x: 0, z: 20 }
      : edge === 'south' ? { x: 0, z: 4 }
        : edge === 'east' ? { x: 8, z: 12 }
          : edge === 'west' ? { x: -8, z: 12 }
            : { x: 0, z: 9 };
  const waitTicks = Math.min(26, Math.max(0, Math.ceil(view.now.blastReadyInMs / (1000 / 30))));
  return [
    ...Array.from({ length: waitTicks }, () => ({ verb: 'SET_WEAPON', weapon: 'rig' })),
    { verb: 'BLAST_AT', pos },
    { verb: 'BLAST_AT', pos },
  ];
}
...
function selfCheck() {
  ...
  assert.equal(blastOrders({ now: { blastReadyInMs: 500, threats: { alive: 1, edge: 'north' } } }).at(-2).verb, 'BLAST_AT');
  assert.deepEqual(PALISADES[0], { x: 0, z: 11 });
}
