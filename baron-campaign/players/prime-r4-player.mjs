...
function blastOrders(view, boss = false) {
  ...
  // One standing order executes per 30 Hz fixed tick. Spend otherwise idle
  // cooldown ticks in rig mode, then use a duplicate Blast as the next probe.
  // Capping this batch below the 32-order door limit leaves room for a draft,
  // repair, move, or build prefix without slicing away both Blast orders.
  const waitTicks = Math.min(26, Math.max(0, Math.ceil((view.now.blastReadyInMs ?? 0) / (1000 / 30))));
  const waits = Array.from({ length: waitTicks }, () => ({ verb: 'SET_WEAPON', weapon: 'rig' }));
  return [...waits, blast, blast];
}
...
function selfCheck() {
  ...
  assert.equal(blastOrders({ now: { blastReadyInMs: 2500, threats: { alive: 1 } } }, true)[0].weapon, 'rig');
