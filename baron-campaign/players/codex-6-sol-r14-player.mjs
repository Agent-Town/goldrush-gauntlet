...
const BEACONS = [{ x: 0, z: 15 }, { x: -6, z: 14 }, { x: -4, z: 15 }, { x: 2, z: 15 }, { x: 3, z: 14 }];
const PALISADES = [{ x: 0, z: 11 }, { x: -10, z: 11 }, { x: 10, z: 11 }];
const STOCKPILES = [{ x: -10, z: 14 }, { x: 10, z: 14 }];
const HOLD = { x: 0, z: 12 };
const COSTS = { sentry_beacon: [25, 35, 45, 55, 75], sluice: 40, turret: [50, 70, 95, 125], stockpile: 60, palisade: 10 };
...
function blastOrders(view, boss = false) {
  ...
  const blast = { verb: 'BLAST_AT', pos: blastTarget(view, boss) };
  return [
...
function selfCheck() {
  ...
  const orders = blastOrders({ now: { blastReadyInMs: 1000, threats: { alive: 1, edge: 'north' } } });
  assert.equal(orders.length, 28);
  assert.ok(orders.slice(0, 26).every(({ verb, weapon }) => verb === 'SET_WEAPON' && weapon === 'rig'));
  assert.deepEqual(orders.at(-1).pos, { x: 0, z: 20 });
  assert.deepEqual(blastOrders({ now: { blastReadyInMs: 0, threats: { alive: 1 } } }, true)[0].pos, { x: 0, z: 11 });
