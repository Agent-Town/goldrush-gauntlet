...
def decide(view):
    ...
    # Aggressive repair: send multiple repair orders to catch all damaged buildings
    works_hp = works['hp']
    works_max = works['maxHp']
    if works_max > 0:
        pct = (works_hp / works_max) * 100
        # Send up to 4 repair orders at high threshold
        repair_threshold = 100
        if pct < 100 or works.get('wrecked', 0) > 0:
            for _ in range(4):
                orders.append({"verb": "REPAIR_UNDER", "pct": repair_threshold})
...
def main():
    for line in sys.stdin:
        ...
        if data.get('schema') == 'goldrush.view.v1':
            ...
            views.append(data)
            with open('test_v4_views.json', 'w') as f:
                json.dump(views, f)
